...
def get_llm():
    return ChatGroq(model="llama-3.1-8b-instant", temperature=0.3, max_tokens=400,
...
def get_articles(plant_name):
    ...
    if TAVILY_API_KEY:
        ...
        res = TavilyClient(api_key=TAVILY_API_KEY).search(
            query, max_results=5, include_raw_content=True)
...
def build_vs(plant_name, articles):
    emb     = load_embeddings()
    persist = f"./chroma_db/{plant_name.replace(' ', '_')}"
...
".join(d.page_content for d in docs)
    prompt = ChatPromptTemplate.from_messages([
        ("system",
         "You are PlantAI. Current plant: {plant_name}. "
...
with st.sidebar:
    ...
    st.divider()
    uploaded = st.file_uploader("Prześlij zdjęcie rośliny", type=["jpg","jpeg","png","webp"])
    if uploaded:
    ...
    with st.chat_message("assistant"):
        if not st.session_state.vectorstore:
            ...
            with st.spinner("Szukam odpowiedzi..."):
                ans = get_answer(q, st.session_state.vectorstore, st.session_state.plant_name)
        st.markdown(ans)
